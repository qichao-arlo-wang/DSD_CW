...
//------------------------------------------------------------------------------
// Purpose:
//   Functional regression for the optimized stateless Task 8 CI block
//   task8_ci_f2_accum_opt.
//
// Coverage:
//   - Software-style chained accumulation: acc_{k+1} = acc_k + f(x_k)
//   - Deterministic vectors and randomized vectors
//   - Stable latency for repeated calls under the default setup
//------------------------------------------------------------------------------
module tb_task8_ci_f2_accum_opt;
    logic clk;
    logic reset;
    logic clk_en;
    logic start;
    ...
    initial begin
        ...
        for (i = 0; i < 8; i = i + 1) begin
            ...
            check_close("seq_accum_opt", got, sum_ref);
            if (first_cycles < 0) begin
                first_cycles = cycles;
            end else if (cycles != first_cycles) begin
                $fatal(1, "Latency changed across deterministic sequence: first=%0d current=%0d",
                       first_cycles, cycles);
            end
        ...
        for (i = 0; i < 20; i = i + 1) begin
            ...
            sum_ref = sum_ref + ref_f(x_real);
            call_accum(acc_bits, x_bits, got, got_bits, cycles);
            check_close("rand_accum_opt", got, sum_ref);
            if (cycles != first_cycles) begin
                $fatal(1, "Latency changed across randomized sequence: first=%0d current=%0d",
                       first_cycles, cycles);
            end
            total_cycles = total_cycles + cycles;
            acc_bits = got_bits;
        ...
        $display("tb_task8_ci_f2_accum_opt latency_cycles=%0d", first_cycles);
        $display("tb_task8_ci_f2_accum_opt total_cycles=%0d", total_cycles);
        $display("tb_task8_ci_f2_accum_opt PASSED");
        $finish;
    end
endmodule
