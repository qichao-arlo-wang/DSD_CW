...
module tb_task8_ci_fsum_pipe_hwish_repeats;
    ...
    initial begin
        ...
        for (run_idx = 0; run_idx < NUM_RUNS; run_idx = run_idx + 1) begin
            ...
            if (abs_err > 1.0e4) begin
                $fatal(1, "repeat run=%0d got=%f ref=%f abs_err=%e", run_idx, got_sum, ref_sum, abs_err);
            end
            $display("[tb_task8_ci_fsum_pipe_hwish_repeats] run=%0d got=%f ref=%f abs_err=%e", run_idx, got_sum, ref_sum, abs_err);
        end

        $display("[tb_task8_ci_fsum_pipe_hwish_repeats] PASS");
        $finish;
    end
endmodule
