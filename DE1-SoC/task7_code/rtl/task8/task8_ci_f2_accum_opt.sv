//------------------------------------------------------------------------------
// Purpose:
//   Board-robust optimized Task-8 custom instruction for software-managed
//   accumulation.
//
// Interface Contract:
...
//   Keep the Task-8 datapath flattened so three independent activities overlap
...
//
// Robustness Note:
//   This version intentionally avoids same-cycle operand bypass launch. All
//   operator inputs are first registered, then the FP/CORDIC blocks are started
//   on the following cycle. This is slightly less aggressive than the 19-cycle
//   experimental variant, but it matches board integration more reliably.
//------------------------------------------------------------------------------
        ...
        if (reset) begin
            ...
            case (state)
                ...
                S_WAIT_TERM: begin
                    ...
                    if (mul_done) begin
                        if (partial_ready) begin
                            add_a       <= partial_sum_reg;
                            add_b       <= mul_result;
                            add_start_r <= 1'b1;
                            state       <= S_WAIT_FINAL;
                        end else begin
