...
module task7_ci_f_single #(
    ...
    // Detailed note:
    // This block optimizes single-sample latency of f(x), not vector throughput.
    // Task 8 is where throughput-oriented pipelining is expected.
    // Step-3 final architecture (single custom instruction for f(x)):
    //   f(x) = 0.5*x + x^3*cos((x-128)/128)
    //
    // Design intent:
    ...
    // Registered intermediates kept across FSM states.
    logic [31:0] x_fp_reg;       // Original input x (fp32)
    logic [31:0] x3_fp_reg;      // x^3 (fp32)
    logic [31:0] half_x_fp_reg;  // 0.5*x (fp32)
    logic [31:0] cos_fp_reg;     // cos((x-128)/128) converted back to fp32

    // Set when CORDIC finishes before x^3 is ready.
    // Prevents losing a one-cycle done pulse and allows deferred consumption.
    logic cos_ready;
    ...
    function automatic logic signed [CORDIC_W-1:0] angle_to_cordic(
        ...
        begin
            ...
            // Constrain input to the validated CORDIC interval [-1, 1].
    ...
    function automatic [31:0] fp32_mul_half(input logic [31:0] x);
        ...
        begin
            ...
            // Implement x*0.5 directly in fp32 domain:
            // - For normal numbers, decrement exponent by 1.
            // - For exp=1 and subnormals, explicitly handle denormal transition
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            // Start signals are one-cycle pulses generated by the FSM.
            // Sub-blocks latch inputs on start and assert done later.
            cordic_start <= 1'b0;
            mul_start    <= 1'b0;
            add_start    <= 1'b0;
            ...
            case (state)
                S_IDLE: begin
                    ...
                    if (start && !cordic_busy && !mul_busy && !add_busy) begin
                        ...
                        // - multiplier computes x^2
                        // - CORDIC computes cos term argument
                        mul_a     <= dataa;
                        mul_b     <= dataa;
                        mul_start <= 1'b1;
                ...
                S_WAIT_X2: begin
                    // CORDIC and x^2 are running in parallel; cache cos() if available.
                    ...
                    if (mul_done) begin
                        // x^3 depends on x^2 result: launch x^3 = x^2 * x.
                        mul_a      <= mul_result;
                        mul_b      <= x_fp_reg;
                        mul_start  <= 1'b1;
                        state      <= S_WAIT_X3;
                ...
                S_WAIT_X3: begin
                    // Keep watching CORDIC in case it finishes during x^3 computation.
                    if (cordic_done) begin
                        cos_fp_reg <= cos_fp_wire;
                        cos_ready  <= 1'b1;
                    end
                    ...
                    if (mul_done) begin
                        ...
                        // If cosine is already ready, chain directly into term multiply.
                        // Otherwise move to WAIT_COS and hold x^3 in x3_fp_reg.
                ...
                S_WAIT_COS: begin
                    // x^3 is ready; only the cosine value is missing.
                    if (cordic_done) begin
                        cos_fp_reg <= cos_fp_wire;
                        cos_ready  <= 1'b1;

                        // Build x^3*cos(...) term as soon as CORDIC completes.
                        mul_a      <= x3_fp_reg;
                        mul_b      <= cos_fp_wire;
                        mul_start  <= 1'b1;
                        state      <= S_WAIT_TERM;
                ...
                S_WAIT_ADD: begin
                    if (add_done) begin
                        // Latch final output before pulsing done.
                        result <= add_result;
                        // Separate output state keeps done asserted cleanly for one cycle.
                        state <= S_OUT;
                    end
                end
