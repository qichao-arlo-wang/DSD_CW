//------------------------------------------------------------------------------
// Purpose:
//   FP subtract execution unit for Task 7 control FSMs.
//
// Notes:
...
`else
    ...
    function automatic logic [31:0] fp_sub_model(
        ...
        begin
            ...
            y_r = a_r - b_r;
            fp_sub_model = $shortrealtobits(y_r);
        end
    endfunction
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (start && !busy) begin
                busy           <= 1'b1;
                cnt            <= EFF_LATENCY - 1;
                pending_result <= fp_sub_model(a, b);
            end else if (busy) begin
                if (cnt == 0) begin
                    busy   <= 1'b0;
                    done   <= 1'b1;
