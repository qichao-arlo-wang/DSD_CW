//------------------------------------------------------------------------------
// Purpose:
//   Round-robin multi-lane fp32 multiply stage for Task 8 streaming pipeline.
//
// Design Note:
//   Each lane uses the existing Task 7 start/done wrapper. With LANES set to
//   LATENCY+3, the stage can accept one new input per cycle in steady state.
...
//   parent retire logic sees `done` and clears `lane_pending`.
//------------------------------------------------------------------------------
module task8_fp_mul_rr_stage #(
    parameter int LATENCY = 3,
    parameter int LANES = LATENCY + 3,
    ...
    genvar g;
    generate
        for (g = 0; g < LANES; g = g + 1) begin : gen_mul_lane
